...
parser.add_argument('--split_new', default="True", type=boolean_string,
                    help='split new index file or not')
parser.add_argument('--split_mode', default='bfs', type=str,
                    help='split method, random, bfs or dfs')
parser.add_argument('--train_valid_index_path', default="./data/new_train_valid_index_json/STRING.dfs.fold1.json", type=str,
...
parser.add_argument('--save_path', default="./output/ppi", type=str,
...
parser.add_argument('--epochs', default=300, type=int,   #epochs=300
...
def train(model, graph, ppi_list, loss_fn, LossClass, optimizer, device,
        result_file_path, summary_writer, save_path,
        batch_size=512, epochs=1000, scheduler=None, 
        got=False, balance=1e-5, use_dmt=True):
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps):
            ...
            if got:
                output, latent = model(graph.x, graph.edge_index_got, train_edge_id)
                label = graph.edge_attr_got[train_edge_id]
            else:
                output, latent = model(graph.x, graph.edge_index, train_edge_id)
            ...
            if use_dmt:
                loss_dmt = LossClass(graph.x, graph.x, latent, latent)
                loss += balance * loss_dmt
...
def main():
    ...
    save_path = os.path.join(args.save_path, "gnn_{}_{}_{}_{}_{}_{}".format(args.description, args.split_mode, args.seed, args.use_dmt, args.balance, time_stamp))
    ...
    train(model, graph, ppi_list, loss_fn, LossClass, optimizer, device,
        result_file_path, summary_writer, save_path,
        batch_size=args.batch_size, epochs=args.epochs, scheduler=scheduler, 
        got=args.graph_only_train, balance=args.balance, use_dmt=args.use_dmt)
    print(args)
